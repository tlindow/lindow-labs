...
export default function Portfolio() {
  return (
    <section id="portfolio" className="py-24 px-6 bg-surface-alt">
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {projects.map((p) => (
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="group flex flex-col justify-between rounded-2xl bg-surface border border-border p-6 hover:shadow-lg hover:border-primary/30 transition-all"
            >
              <div>
                <div className="flex items-start justify-between">
                  ...
                  <ExternalLink
                    size={16}
                    className="mt-1 shrink-0 text-muted opacity-0 group-hover:opacity-100 transition-opacity"
                  />
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {p.tech.map((t) => (
                  <span
                    key={t}
                    className="rounded-full bg-surface-alt px-3 py-0.5 text-xs font-medium text-muted"
                  >
                    {t}
                  </span>
                ))}
